...
import { computeRowAmounts, pickRateRule, round2, ZERO_RULE_RATES } from '../../common/payroll/rate-rule.util';
...
export class AnalyticsService {
  ...
  /**
   * Участки, доступные пользователю для аналитики: company-scope видит
   * все участки компании, own_sites — только перечисленные в
   * RoleAssignment.siteIds. requestedSiteId дополнительно сужает выбор
   * и должен входить в уже разрешённый список — иначе 403 (нельзя
   * обойти scope, просто явно запросив чужой участок).
   */
  private async resolveAccessibleSiteIds(user: KernUser, requestedSiteId?: string): Promise<string[]> {
    ...
    if (perms.some((p) => p.scope === 'company')) {
      resolved = companySiteIds;
    } else {
      const ownSites = perms.find((p) => p.scope === 'own_sites');
      resolved = ownSites ? companySiteIds.filter((id) => ownSites.siteIds.includes(id)) : [];
    }

    if (requestedSiteId) {
      if (!resolved.includes(requestedSiteId)) {
  ...
  /** Стоимость одной строки табеля с дедупликацией суточных по (сотрудник, дата) — как в PayrollCalcService. */
  private rowCost(t: TimesheetForAnalytics, rules: RateRule[], perDiemSeen: Set<string>): Prisma.Decimal {
  ...
  async timeseries(user: KernUser, from: Date, to: Date, grain: Grain, siteId?: string) {
    const siteIds = await this.resolveAccessibleSiteIds(user, siteId);
    const { timesheets, rules } = await this.loadTimesheetsForCompany(user.companyId, siteIds, from, to);
    ...
    for (const t of timesheets) {
      ...
      const cost = this.rowCost(t, rules, perDiemSeen);
      ...
      bucket.metersDrilled = bucket.metersDrilled.plus(t.metersDrilled ?? new Prisma.Decimal(0));
  ...
  async totals(user: KernUser, from: Date, to: Date, siteId?: string) {
    ...
    for (const t of timesheets) {
      ...
      totals.metersDrilled = totals.metersDrilled.plus(t.metersDrilled ?? new Prisma.Decimal(0));
  ...
  async budgetVsActual(user: KernUser, from: Date, to: Date, siteId?: string) {
    ...
    const [sites, { timesheets, rules }] = await Promise.all([
      ...
      this.loadTimesheetsForCompany(user.companyId, siteIds, from, to),
    ]);
  ...
  private async loadTimesheetsForCompany(companyId: string, siteIds: string[], from: Date, to: Date) {
    ...
    const [timesheets, rules] = await Promise.all([
      this.prisma.timesheet.findMany({
        where: { siteId: { in: siteIds }, status: { in: CONFIRMED_STATUSES }, workDate: { gte: from, lt: to } },
        include: { employee: { include: { position: true } } },
      }),
